;;; doom-dashboard.el -*- lexical-binding: t; -*-

;;; Commentary:

;; Doom dashboard as a standalone package
...
(defun split-init ()
  "Split the window, switch to the new window, and open the init file."
  (interactive)
  (split-window-right)
  (other-window 1)
...
(defun open-doom-dashboard-if-scratch-buffer-killed ()
  "Open Doom dashboard if the scratch buffer is killed."
  (when (string= (buffer-name) "*scratch*")
    (+doom-dashboard/open (selected-frame))))

(add-hook 'kill-buffer-hook 'open-doom-dashboard-if-scratch-buffer-killed)


(defun prevent-kill-doom-dashboard ()
...
(add-hook 'kill-buffer-query-functions 'prevent-kill-doom-dashboard)


(defun split-init-directory ()
  "Open the directory containing the `user-init-file`."
  (interactive)
  (split-window-right)
  (other-window 1)
...
If RETURN-P, return the message as a string instead of displaying it."
  (funcall (if return-p #'format #'message)
           "Doom loaded 164 packages across 33 modules in 0.625s"))
...
(defvar doom-fallback-buffer-name "*fallback*"
...
(defface doom-dashboard-menu-title '((t (:inherit font-lock-keyword-face :weight regular)))
...
(defvar doom-version "3.0.0"
  "The current version of Doom Emacs.")
...
(define-key +doom-dashboard-mode-map (kbd "n") 'forward-button)
(define-key +doom-dashboard-mode-map (kbd "p") 'backward-button)
(define-key +doom-dashboard-mode-map (kbd "j") 'forward-button)
(define-key +doom-dashboard-mode-map (kbd "k") 'backward-button)
(define-key +doom-dashboard-mode-map (kbd "C-n") 'forward-button)
(define-key +doom-dashboard-mode-map (kbd "C-p") 'backward-button)
(define-key +doom-dashboard-mode-map [down] 'forward-button)
(define-key +doom-dashboard-mode-map [up] 'backward-button)
(define-key +doom-dashboard-mode-map [tab] 'forward-button)
(define-key +doom-dashboard-mode-map [backtab] 'backward-button)

;; Evil remaps
(define-key +doom-dashboard-mode-map [remap evil-next-line] 'forward-button)
(define-key +doom-dashboard-mode-map [remap evil-previous-line] 'backward-button)
(define-key +doom-dashboard-mode-map [remap evil-next-visual-line] 'forward-button)
(define-key +doom-dashboard-mode-map [remap evil-previous-visual-line] 'backward-button)
(define-key +doom-dashboard-mode-map [remap evil-paste-pop-next] 'forward-button)
(define-key +doom-dashboard-mode-map [remap evil-paste-pop] 'backward-button)
(define-key +doom-dashboard-mode-map [remap evil-delete] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-delete-line] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-insert] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-append] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-replace] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-enter-replace-state] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-change] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-change-line] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-visual-char] 'ignore)
(define-key +doom-dashboard-mode-map [remap evil-visual-line] 'ignore)
   ...
   (+doom-dashboard--center
    ...
    (with-temp-buffer
      (insert-text-button (or (nerd-icons-codicon "nf-cod-octoface" :face 'doom-dashboard-footer-icon :height 1.3 :v-adjust -0.15)
                          ...
                          'action (lambda (_) (browse-url "https://github.com/laluxx/minimacs"))
                          'follow-link t
...
;; TODO variable to decide if we want to wrap or not

;;;###autoload
(defun +doom-dashboard/forward-button (n)
  "Like `forward-button', but also echo."
  (interactive "p")
  (forward-button n t)
...
(defun +doom-dashboard/backward-button (n)
  "Like `backward-button', but also echo."
  (interactive "p")
  (backward-button n t)
  (+doom-dashboard--help-echo))
